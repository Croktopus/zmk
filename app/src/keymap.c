...
int zmk_keymap_remove_layer(zmk_keymap_layer_index_t index) {
    ...
    if (keymap_layer_orders[index] == UINT8_MAX) {
        return -EINVAL;
    }
    ...
    keymap_layer_orders[ZMK_KEYMAP_LAYERS_LEN - 1] = UINT8_MAX;
...
int zmk_keymap_set_layer_name(zmk_keymap_layer_id_t id, const char *name, size_t size) {
    ...
    strlcpy(zmk_keymap_layer_names[id], name, MIN(size, CONFIG_ZMK_KEYMAP_LAYER_NAME_MAX_LEN));
...
static void save_layer_orders(void) {
    settings_save_one("keymap/layer_order", keymap_layer_orders, ARRAY_SIZE(keymap_layer_orders));
...
static void save_layer_names(void) {
    for (int id = 0; id < ZMK_KEYMAP_LAYERS_LEN; id++) {
        if (changed_layer_names & BIT(id)) {
            ...
            sprintf(setting_name, "keymap/l_n/%d", id);
...
int zmk_keymap_discard_changes(void) {
    ...
    int ret = settings_load_subtree("keymap/l");
    if (ret >= 0) {
        changed_layer_names = 0;
...
int keymap_init(void) {
    ...
    while (i < ZMK_KEYMAP_LAYERS_LEN) {
        keymap_layer_orders[i] = UINT8_MAX;
        i++;
    }
#endif
